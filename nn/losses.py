...
class PhysicsInformedFlowMatchingLoss(nn.Module):
    r"""Physics-informed Flow Matching Loss
    """
    def __init__(self, scale_0 = 3,div_penalty: float = 0.1, eps: float = 1e-6):
        super().__init__()
    ...
    def forward(
        ...
        pred_v = model(graph)  # (num_nodes, num_fields)
        target = graph.advection_field
        assert pred_v.shape == target.shape, \
            f"pred_v.shape={pred_v.shape}, advection_field.shape={target.shape}"
        loss_flow = batch_wise_mean((pred_v - target)**2, graph.batch)  # (batch_size,)

        src, dst = graph.edge_index
        pos = graph.pos  # (num_nodes, dim)
        vf = pred_v     # (num_nodes, dim)，要求 num_fields == dim
        if pos.shape[0] != vf.shape[0]:
            pos = getattr(graph, f'pos_{self.scale_0}')
        
        edge_vec = pos[dst] - pos[src]                                 # (E, dim)
        edge_len = torch.norm(edge_vec, dim=1, keepdim=True) + self.eps
        edge_dir = edge_vec / edge_len                                 # (E, dim)
        
        flux_src = (vf[src] * edge_dir).sum(dim=1)                     # (E,)
        flux_dst = (vf[dst] * edge_dir).sum(dim=1)                     # (E,)
        
        div = torch.zeros(pos.size(0), device=pos.device)              # (num_nodes,)
        div.index_add_(0, dst, flux_dst)
        div.index_add_(0, src, -flux_src)
        
        deg = torch.zeros_like(div).index_add_(0, dst, torch.ones_like(dst, dtype=torch.float))
        deg = deg.index_add_(0, src, torch.ones_like(src, dtype=torch.float))
        div = div / (deg + self.eps)
        
        loss_div = batch_wise_mean(div**2, graph.batch)                # (batch_size,)

        return loss_flow + self.div_penalty * loss_div