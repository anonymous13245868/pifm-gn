...
class ConservationFlowMatchingGraphNet(FlowMatchingModel):
    ...
    def load_arch(self, arch: dict):
        ...
        # )
        # 输出投影层(如需要调整维度)
        # self.output_projection = nn.Linear(self.dim, self.out_node_features)
    ...
    def forward(self, graph: Graph) -> torch.Tensor:
        ...
        vector_field = self.node_decoder(v)   # (num_nodes, out_node_features)
        # 预测势函数 - 替代直接解码
        # potential = self.potential_network(v)
        
        # # 应用旋度算子确保无散度
        # vector_field = graph_curl_operator(potential, graph.edge_index, graph.pos)
    
        # # 总是将3D向量场投影回期望的输出维度
        # if vector_field.size(1) != self.out_node_features:
        #     # 如果需要，动态创建投影层
        #     if not hasattr(self, 'output_projection'):
        #         self.output_projection = nn.Linear(
        #             vector_field.size(1),  # 通常是3
        #             self.out_node_features,  # 与输入维度相同
        #             device=vector_field.device
        #         ).to(vector_field.device)
        #     vector_field = self.output_projection(vector_field)
