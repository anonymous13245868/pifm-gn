...
def pos_field(
    ...
    if dim == 2: 
        ...
        ax = fig.add_subplot(projection='3d')
        im = ax.scatter(pos[:,0], pos[:,1], pos[:,2], s=s, c=u, cmap=cmap, vmin=vmin, vmax=vmax)
        ax.set_xlabel('x', fontsize=fontsize, labelpad=1)
        ax.set_ylabel('y', fontsize=fontsize, labelpad=1)
        ax.set_zlabel('z', fontsize=fontsize, labelpad=0, rotation=0)  # rotation=0使z标签水平
        if azim is not None: ax.view_init(azim=azim)
        if dist is not None: ax.dist = dist
        if elev is not None: ax.elev = elev
        # Remove background
    ...
    if label:
        if label['text']:
        # 使用fig.text代替ax.text
            fig.text(label['x'], label['y'], label['text'], fontsize=label['fontsize'], 
                    bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.2'))
        if label['title']:
            ax.set_title(label['title'], fontsize=label['fontsize']+5, pad=-1,y=0.95)
    cax = fig.add_axes([ax.get_position().x1,ax.get_position().y0+0.1,0.02,ax.get_position().height-0.2])
    plt.colorbar(im, cax=cax)
    cax.yaxis.set_tick_params(labelsize=13)  # 将colorbar字体从20改为12
    if filename:
        fig.savefig(filename)
    plt.show()
    return fig, ax
